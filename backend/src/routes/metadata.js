  ...
  async (req, res) => {
    try {
      ...
      const collectionMetadata = {
        ...
        description:
          'Decentralized proof certificates secured by blockchain technology and IPFS storage. Each NFT represents a verified digital proof with cryptographic authenticity.',
...
router.get(
  ...
  [
    query('type')
      .optional()
      .isIn(['basic', 'educational', 'professional', 'identity']),
  ],
  async (req, res) => {
    try {
      ...
      res.status(200).json({
        ...
        data: {
          ...
          instructions: {
            ...
            'IPFS Hash':
              'Upload certificate image/document to IPFS and use the hash',
            'Token ID': 'Use the actual NFT token ID after minting',
            'Custom Attributes':
              'Add more attributes as needed for your use case',
          },
          availableTypes: Object.keys(templates),
        },
      });
