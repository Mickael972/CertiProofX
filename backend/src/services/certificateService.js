...
class CertificateService {
  ...
  async generateCertificatePDF(certificateData) {
    try {
      ...
      const qrCodeBuffer = await this.generateQRCode(
        verificationUrl || `https://certiproof-x.com/verify/${tokenId}`
      );
  ...
  async addCertificateContent(doc, data) {
    ...
    doc
      .font(config.certificate.pdf.font.regular)
      .text(
        data.documentType || 'Digital Document',
        leftColumn + 100,
        currentY
      );
    ...
    if (data.recipientAddress) {
      ...
      doc
        .font(config.certificate.pdf.font.regular)
        .fontSize(10)
        .text(data.recipientAddress, rightColumn, currentY + 15, {
          width: 200,
        });
    }
    ...
    doc
      ...
      .text(
        'This certificate is secured by blockchain technology and',
        70,
        currentY
      );
    doc.text(
      'cryptographic proof. Verification can be performed at any time.',
      70,
      currentY + 12
    );
    ...
    doc
      .fontSize(8)
      .text(
        `Generated by CertiProof X on ${new Date().toISOString()}`,
        70,
        currentY
      );
    doc.text(
      `Author: ${config.app.author} | Contact: ${config.app.contact}`,
      70,
      currentY + 10
    );
  }
  ...
  async addFooter(doc) {
    ...
    doc.text(
      `License: ${config.app.license} | Repository: ${config.app.repository}`,
      50,
      footerY + 25
    );

    doc.text(`Page 1 of 1`, 495, footerY + 15, { align: 'right' });
    doc.text(`Generated: ${new Date().toISOString()}`, 300, footerY + 25, {
      align: 'right',
    });
  }
  ...
  validateCertificateData(data) {
    ...
    if (
      ...
      !/^0x[a-fA-F0-9]{40}$/.test(data.recipientAddress)
    ) {
